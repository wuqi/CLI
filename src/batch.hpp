...
namespace batch
{
  ...
  public:
    ...
    {
      std::string pt = pattern;
      dir_and_mask_ = SplitPath (pt);
    ...
    inline std::string GetFileName (bool withPath = true) const
    {
      assert (ok_);

      if (withPath) {
        std::string out = dir_and_mask_.first;
        out += "/";
        out += find_data_.cFileName;
        return out;
      } else {
  ...
  private:
    ...
    std::pair<std::string, std::string> dir_and_mask_;
  };
#else
  class Glob
  {
  public:
        ...
        dir_entry_ (0)
    {
      std::string pt = pattern;
      dir_and_mask_ = SplitPath (pt);
      dir_ = opendir (dir_and_mask_.first.c_str());
      pattern_ = dir_and_mask_.second;
